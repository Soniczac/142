...
def recommended_articles():
    ...
    all_recommened = pd.DataFrame(columns=col_names)

    for liked_articales in liked_articles:
        output = get_recommendations(liked_articles["original_title"])
        all_recommened=all_recommened.append(output)
        all_recommened.drop_duplicates(subset=["original_title"])

        recommended_articales_data=[]

        for index, row in all_recommened.iterrows():
            _p = {
                 "original_title": row["original_title"],
            "poster_link":row['poster_link'],
            "release_date":row['release_date'] or "N/A",
            ...
            recommended_articales_data.append(_p)

    return jsonify({
        "data":recommended_articales_data,
        "status": "success"
    })
